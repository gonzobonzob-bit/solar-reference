...
/* Build equipment map from shared EQUIPMENT_DATA — ESS/hybrid/string only.
   Adds .label/.amps/.note aliases so existing render code works unchanged. */
const EQUIPMENT = (function() {
  const result = {};
  Object.entries(EQUIPMENT_DATA).forEach(([mfgKey, mfg]) => {
    const models = {};
    Object.entries(mfg.models).forEach(([modelKey, m]) => {
      if (m.type === 'micro' || m.offgridOnly || m.nonStandardUS) return;
      models[modelKey] = Object.assign({}, m, {
        label: m.name,
        amps:  m.acOutputCurrent,
        note:  m.notes || ''
      });
    });
    if (Object.keys(models).length > 0) {
      result[mfgKey] = { label: mfg.name, models };
    }
  });
  return result;
})();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Read URL parameters from pv-sizing handoff
  readUrlParams();
});

/* ─── URL PARAMETER INTAKE (from pv-sizing tool) ───────────────────── */
...
function readUrlParams() {
  ...
  const acOutput = params.get('acOutput');
  const mfg      = params.get('manufacturer');
  const model    = params.get('model');
  const source   = params.get('source');

  if (bus) {
    const v = parseFloat(bus);
    if (v > 0) { el('busRating').value = v; state.busRating = v; }
  }
  if (main) {
    const v = parseFloat(main);
    if (v > 0) { el('mainBreaker').value = v; state.mainBreaker = v; }
  }
  if (acOutput) {
    const v = parseFloat(acOutput);
    if (v > 0) { el('essOutputCurrent').value = v; state.essOutputCurrent = v; }
  }
  if (mfg && EQUIPMENT[mfg]) {
    state.eqMfg = mfg;
    const mfgSel = el('eqMfgSelect');
    if (mfgSel) mfgSel.value = mfg;
    if (model && EQUIPMENT[mfg].models[model]) {
      state.eqModel = model;
    }
  }
